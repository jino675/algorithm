...
char	completed_tree[201];
int		N;

int	in_order_check(int cur)
{
	int	left;
	int	right;
	...
	if (right <= N) {
		...
		if (completed_tree[cur] < '0' || completed_tree[cur] > '9')
			return (-1);
	}
	if (left <= N)
		if (in_order_check(left) == -1)
			return (-1); 
	if (right <= N)
		if (in_order_check(right) == -1)
			return (-1); 
	return (0);
}

// void	print_completed_tree()
// {
// 	int	n;
// 	int idx;
// 	int	height;
...
// 	for (int j = 0; j < 7; ++j) {
// 		printf("level %d : ", j);
// 		for (int q = 0; q < (1 << height) / (1 << j); ++q)
// 			printf(" ");
// 		for (int k = 0; k < (1 << j); ++k) {
// 			printf("%c", completed_tree[++idx]);
// 			for (int q = 0; q < (1 << ((height + 1) - j)) - 1; ++q)
// 				printf(" ");
// 		}
// 		printf("\n");
// 	}
...
int	main(void)
{
	...
	for (int i = 0; i < 10; ++i) {
		...
		// print_completed_tree();
		printf("#%d ", i + 1);
		if (in_order_check(1) == -1)
			printf("0");
		else
			printf("1");
		printf("\n");
