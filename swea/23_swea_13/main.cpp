...
static void makeTree(int n) {
	...
	c[pseudo_rand(MAX_K + 1) + 1] = 0;	//root node
	for (int i = 0; i < n; ++i) {
		int pi = pseudo_rand(MAX_K + 1) + 1;	//자식을 가질 수 있는 놈
		while (c[pi] < 0 || c[pi] >= MAX_CHILD) {
			++pi;
			if (pi == MAX_K + 2)
				pi = 1;
		}
		int ci = pseudo_rand(MAX_K + 1) + 1;	//아직 태어나지 않은 놈
		while (c[ci] >= 0) {
			++ci;
			if (ci == MAX_K + 2)
				ci = 1;
	...
	for (int i = 0; i < n; ++i) {
		int e = pseudo_rand(MAX_K + 1) + 1;		//아직 출력되지 않은 놈
