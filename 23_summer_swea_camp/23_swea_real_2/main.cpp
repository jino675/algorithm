...
static int run()
{
	...
	for (int c = 0; c < N; ++c)
	{
		...
		case CMD_ALLY:
			...
			scanf("%d", &check);
			if (result != check) {
				printf("ALLY : %s %s, ans : %d, res : %d\n", mMonarchA, mMonarchB, check, result);
				sleep(100);
				isOK = 0;
			}
			break;
		...
		case CMD_ATTACK:
			...
			scanf("%d", &check);
			if (result != check) {
				printf("ATTACK : %s %s %s, ans : %d, res : %d\n", mMonarchA, mMonarchB, mGeneral, \
																	check, result);
				sleep(100);
				isOK = 0;
			}
		...
		case CMD_RECRUIT:
			...
			scanf("%d", &check);
			if (result != check) {
				printf("RECRUIT : %s %d %d, ans : %d, res : %d\n", mMonarchA, num, mOption, check, result);
				sleep(100);
				isOK = 0;
			}
			break;
