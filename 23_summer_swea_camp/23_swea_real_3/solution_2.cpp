...
priority_queue<int, vector<int>, greater<int> >	time_table[60][60][24];
priority_queue<int, vector<int>, greater<int> >	price_table[60][60];
...
void init(int N)
{
	for (int i = 0; i < port_cnt; ++i) {
		for (int j = 0; j < port_cnt; ++j) {
			while (price_table[i][j].empty() != true)
				price_table[i][j].pop();
			for (int k = 0; k < 24; ++k)
				while (time_table[i][j][k].empty() != true)
					time_table[i][j][k].pop();
		}
	}
	port_cnt = N;
}
...
void add(int mStartAirport, int mEndAirport, int mStartTime, int mTravelTime, int mPrice)
{
	...
	price_table[mStartAirport][mEndAirport].push(mPrice);
	for (int i = 0; i < 24; ++i) {
		temp_val = mStartTime - i;
		if (temp_val < 0)
			temp_val += 24;
		temp_val += mTravelTime;
		time_table[mStartAirport][mEndAirport][i].push(temp_val);
	}
}
...
void	update_table_time(int cur_port, int cur_time)
{
	...
	for (next_port = 0; next_port < port_cnt; ++next_port) {
		if (visited[next_port] == 0 && time_table[cur_port][next_port][cur_time].empty() != true) {
			temp_val = time_table[cur_port][next_port][cur_time].top();
			temp_val += min_table[cur_port];
			if (temp_val < min_table[next_port]) {
				min_table[next_port] = temp_val;
				pq.push({next_port, min_table[next_port]});
			}
		}
...
int minTravelTime(int mStartAirport, int mEndAirport, int mStartTime)
{
	...
	pq.push({mStartAirport, 0});
	while (pq.empty() != true) {
		cur_port = pq.top().first;
		pq.pop();
...
void	update_table_price(int cur_port)
{
	...
	for (next_port = 0; next_port < port_cnt; ++next_port) {
		if (visited[next_port] == 0 && price_table[cur_port][next_port].empty() != true) {
			temp_val = price_table[cur_port][next_port].top();
			temp_val += min_table[cur_port];
			if (temp_val < min_table[next_port]) {
				min_table[next_port] = temp_val;
				pq.push({next_port, min_table[next_port]});
