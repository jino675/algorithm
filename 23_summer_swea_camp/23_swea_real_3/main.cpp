...
static bool run()
{
	...
	for (int q = 0; q < numQuery; q++)
	{
		...
		case CMD_MIN_TRAVEL_TIME:
			...
			if (userAns != ans)
			{
				printf("\n\ncmd : %d, start : %d, end : %d, start_time : %d, ans : %d, res : %d\n", \
							cmd, mStartAirport, mEndAirport, mStartTime, ans, userAns);
				sleep(100);
				isCorrect = false;
			}
			break;
		case CMD_MIN_PRICE:
			...
			if (userAns != ans)
			{
				printf("\n\ncmd : %d, start : %d, end : %d, ans : %d, res : %d\n", \
							cmd, mStartAirport, mEndAirport, ans, userAns);
				sleep(100);
				isCorrect = false;
			}
			break;
